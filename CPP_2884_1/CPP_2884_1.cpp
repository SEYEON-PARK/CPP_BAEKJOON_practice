...
{
    int H, M; // 변수 선언
    cin >> H >> M; // 사용자로부터 두 개의 정수 입력받기
    
    if(M>=45) // 만약, M이 45보다 크거나 같다면
    {
        M=M-45; // M에 M-45 값 대입하기
    }
    else if(H==0) // 만약 앞의 조건이 맞지 않고, H가 0과 같다면 
     {
        M=60-(45-M); // M에 60-(45-M) 값 대입하기
        H=23; // H에 23 대입하기
    }
    else // 앞의 조건들 다 맞지 않는다면
    {
        M=60-(45-M); // M에 60-(45-M) 값 대입하기
        H--; // H에 1 빼기
    }
